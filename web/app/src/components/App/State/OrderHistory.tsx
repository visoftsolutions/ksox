      ...
      <Index each={store}>
        {(element, i) => (
          <div class={`grid grid-cols-8 items-center self-center px-[12px] py-[8px] text-state-item font-normal text-white ${i % 2 && "bg-gray-3"} `}>
            ...
            <div class="col-start-5 col-end-6 text-center">{ev(fFromWei(element().order_value))}</div>
            <div class="col-start-6 col-end-7 text-center">{ev(fFromWei(element().order_qty))}</div>
            <div class="col-start-7 col-end-8 text-center">{ev(fFromWei(element().filled_qty))}</div>

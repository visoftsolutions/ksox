...
import { CONTRACT_ON_CHAIN, crowdsale, setCrowdsale } from "~/components/providers/CrowdsaleProvider";
...
export function TermsModal() {
  ...
  return (
    ...
    >
      <div class="m-auto grid h-[500px] w-[50%] grid-flow-row gap-8 rounded-xl bg-[#000033] p-4 text-text-1 outline-none" ref={modalDOM}>
        <div class="grid grid-cols-[auto_1fr_auto] items-center justify-center gap-4 text-xl font-bold">
          <img src={joinPaths(base, "/gfx/legal-agreement.svg")} width="30px" elementtiming={""} fetchpriority={"high"} />
        ...
        <div class="overflow-scroll rounded-md border-[1px] border-gray-700 p-2">
          ...
          Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum. Why do we use it? It
          ...
          interested. Sections 1.10.32 and 1.10.33 from "de Finibus Bonorum et Malorum" by Cicero are also reproduced in their exact original form, accompanied
          by English versions from the 1914 translation by H. Rackham.
        </div>

        <div class="mr-4 grid cursor-pointer grid-cols-[auto_1fr] items-center">
          <input id="terms-checkbox" type="checkbox" onChange={(e) => setTermsChecked(e.target.checked)} class="h-4 w-4 cursor-pointer accent-buttonbg_new" />
          <label for="terms-checkbox" class="ml-2 cursor-pointer select-none text-sm font-medium">
          ...
          onClick={async () => {
            if (termsChecked()) {
              ...
              try {
                ...
                const wethContract = CONTRACT_ON_CHAIN.get(wallet.selected_network.network.name)?.wethContract;

                if (wallet.walletClient != undefined && wallet.address != undefined && phaseContract != undefined && wethContract != undefined) {
                  const amount = BigInt(Math.floor(crowdsale.tokenAmount * 10 ** 18));

                  await wallet.walletClient.writeContract({
                    address: wethContract.address,
                    abi: wethContract.abi,
                    functionName: "approve",
                    args: [phaseContract.address, amount],

...
function formatHumanReadable(input: string, precision: number) {
  ...
  let formatted =
    precision > 0 ? input.replace(/[^0-9.]/g, "") : input.replace(/[^0-9]/g, "")
...
export function DepositNumberInput(props: NumberInputComponent) {
  ...
  return (
    <div
      class={` flex justify-between items-center rounded-md bg-gray-2 py-2 w-full ${
        props.class
    ...
    >
      <label
        for={id}
        class=" px-[8px] text-left text-submit-sublabel text-gray-4"
      >
        Amount
      </label>
      <div class="text-right flex items-center">
        <input
          id={id}
          class={"number_input  bg-transparent p-1 text-right outline-none"}
          type="text"
          spellcheck={true}
          ref={inputDOM}
          value={format(

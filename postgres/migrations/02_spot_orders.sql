...
CREATE OR REPLACE FUNCTION spot_orders_orderbook_notify_opposite()
RETURNS TRIGGER AS
$BODY$
DECLARE
  row_data json;
BEGIN
  SELECT json_agg(row_to_json(sub.*)) INTO row_data
      FROM (
        SELECT
            ROUND(CAST(quote_asset_volume/base_asset_volume AS NUMERIC), CAST(TG_ARGV[1] AS INTEGER))::float as price,
            SUM(quote_asset_volume_left)::text as volume
        FROM spot.orders
        WHERE quote_asset_id = NEW.quote_asset_id
        AND base_asset_id = NEW.base_asset_id
...
CREATE FUNCTION create_spot_orders_notify_trigger_for_orderbook_opposite(trigger_name text, quote_asset_id uuid, base_asset_id uuid, precission INTEGER, _limit BIGINT)
RETURNS VOID AS
$BODY$
DECLARE
  trigger_truncated_name text := LEFT(format('t_%s', trigger_name), 63);
...
BEGIN
  ...
  EXECUTE format('
    ...
    FOR EACH ROW
    WHEN (NEW.quote_asset_id = ''%s'' AND NEW.base_asset_id = ''%s'')
    EXECUTE FUNCTION spot_orders_orderbook_notify_opposite(''%s'', %s, %s);', 
    trigger_truncated_name, quote_asset_id::text, base_asset_id::text, channel_truncated_name, precission, _limit);
END;
$BODY$ LANGUAGE plpgsql;

CREATE FUNCTION drop_spot_orders_notify_trigger_for_orderbook_opposite(trigger_name text)
RETURNS VOID AS
$BODY$
DECLARE
  listener_count integer;

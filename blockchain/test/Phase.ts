...
describe("Phase", function () {
  ...
  async function deployAndStart() {
    ...
    await phase.connect(owner).startNewBucket(startTimestamp, 10n**6n * 10n**18n, 10);
  ...
  it("Withdraw Throws When Phase Active", async () => {
    ...
    await expect(phase.withdraw(wethToken.address, otherAccount.address, 100)).to.be.revertedWith("PHASE_ACTIVE");
  });

  it("Start New Bucket Emits Event", async () => {
    const { phase } = await loadFixture(deploy);
    let startTimestamp = Math.round(Date.now() / 1000) + 12 * 3600;
    await expect(phase.startNewBucket(startTimestamp, 10n * 10n**18n, 10)).to.emit(phase, "NewBucketCreated").withArgs(1, startTimestamp, 10n * 10n**18n, 10);
  });

  it("Conclde current bucket", async () => {
    const { phase } = await loadFixture(deployAndStart);
    await expect(phase.concludeCurrentBucket()).to.emit(phase, "BucketConcluded").withArgs(1, 0, 0);
  });

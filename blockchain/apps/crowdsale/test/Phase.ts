...
describe("Phase", function () {
  async function getTokenAt(address: string) {
    return await ethers.getContractAt("contracts/IERC20Metadata.sol:IERC20Metadata", address);
  }
  ...
  async function deployAndStart() {
    ...
    await phase.connect(owner).startNewBucket(startTimestamp, finishTimestamp, 10n ** 6n * 10n ** 18n, 10, 100);
  ...
  it("Conclude Current Bucket Throws When Not Active", async () => {
    ...
    await expect(phase.concludeCurrentBucket()).to.be.revertedWith("BUCKET_NOT_ACTIVE");
  });

  it("Conclude Whole Phase Emits Event", async () => {
    const { phase } = await loadFixture(deploy);
    await expect(phase.concludeWholePhase()).to.emit(phase, "PhaseConcluded").withArgs(0);
  });
  ...
  it("Withdraw Throws When Bucket Active", async () => {
    ...
    let startTimestamp = Math.round(Date.now() / 1000) + 13 * 3600;
    let finishTimestamp = startTimestamp + 12 * 3600;
    await phase.startNewBucket(startTimestamp, finishTimestamp, 10n ** 6n * 10n ** 18n, 10, 100);
    await expect(phase.withdraw(wethToken.address, otherAccount.address, 100)).to.be.revertedWith("BUCKET_ACTIVE");
  });
  ...
  it("Start New Bucket Emits Event", async () => {
    ...
    let finishTimestamp = startTimestamp + 12 * 3600;
    await expect(phase.startNewBucket(startTimestamp, finishTimestamp, 10n * 10n ** 18n, 10, 100))
  ...
  it("Conclde current bucket", async () => {
    ...
    await expect(phase.concludeCurrentBucket()).to.emit(phase, "BucketConcluded").withArgs(1, 0);
  });

  it("But With ETH", async () => {
    const { phase, tokenTicket, owner, otherAccount } = await loadFixture(deploy);
    ...
    expect(await tokenTicket.connect(otherAccount).balanceOf(otherAccount.address)).to.be.equal(18418517370000000000000n);
  });
  ...
  it("But With ETH Whole Bucket", async () => {
    ...
    await phase.connect(owner).startNewBucket(startTimestamp, finishTimestamp, amountToSell, 10, 100);
    await time.setNextBlockTimestamp(startTimestamp);
    expect(await phase.isBucketActive()).to.be.true;
    ...
    expect(await tokenTicket.connect(otherAccount).balanceOf(otherAccount.address)).to.be.equal(amountToSell);
  });
});

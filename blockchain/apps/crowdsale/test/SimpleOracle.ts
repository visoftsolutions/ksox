...
describe("UniswapV3Twap", function () {
  async function deploy() {
    ...
    const simpleOracle = await SimpleOracle.connect(owner).deploy(ETHEREUM_UNISWAP_V3_FACTORY_ADDRESS, ETHEREUM_USDC_ADDRESS, ETHEREUM_WETH_ADDRESS, 3000);
  ...
  it("Basic Test", async () => {
    ...
    const price = await simpleOracle.estimateAmountOut(ETHEREUM_WETH_ADDRESS, 10n ** 18n, 3600);
